// copy of loglevel-plugin-remote.js https://www.npmjs.com/package/loglevel-plugin-remote
  ...
  for (let s = 0; s < arguments.length; s += 1) {
    ...
    for (const key in source) {
      if (hasOwnProperty.call(source, key)) {
        target[key] = typeof source[key] === "object" && !Array.isArray(source[key]) ? assign(target[key], source[key]) : source[key]
      }
    }
  }
  return target
...
function plain(log) {
  return `[${log.timestamp}] ${log.level.label.toUpperCase()}${log.logger ? ` (${log.logger})` : ""}: ${log.message}${
    log.stacktrace ? `\n${log.stacktrace}` : ""
  }`
}
  ...
  apply(logger, options) {
    ...
    function send() {
      ...
      if (!queue.sent()) {
        ...
        // console.log("logMessages", logMessages, JSON.stringify(logMessages))
        // queue.content = isJSON ? `{"logs":[${logs.join(',')}]}` : logs.join('\n');
        // queue.content = isJSON ? `{"logs":${JSON.stringify(logs)}}` : logs.join("\n")
        queue.content = isJSON ? JSON.stringify(logMessages) : logs.join("\n")
      }
